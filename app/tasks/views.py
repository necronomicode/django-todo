    ...
    def get(self, request, task_id=None):
        ...
        if not tasks:
            response = {
                "code": 2,
                "status": "error",
                "body": {"message": "Tasks not found with the specified parameters"},
            }
            return Response(response, status=status.HTTP_404_NOT_FOUND)
        ...
        response = {"code": 200, "status": "ok", "body": {"data": serializer.data}}
    ...
    def get(self, request, category_id=None):
        if category_id:
            category = Category.objects.filter(
                id=category_id, user=request.user.id
            ).first()
    ...
    def get(self, request, tag_id=None):
        ...
        if not tags:
            response = {
                "code": 11,
                "status": "error",
                "body": {"message": "Tags not found with the specified parameters"},
            }
            return Response(response, status=status.HTTP_404_NOT_FOUND)
